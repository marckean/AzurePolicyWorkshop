...
$TSRG = Get-AzResourceGroup -ResourceGroupName $variables.tsResourceGroupName -Location $location
if($null -eq $TSRG){New-AzResourceGroup -Name $variables.tsResourceGroupName -Location $location}
...
New-AzManagementGroupDeployment -Location $variables.location -TemplateFile '.\02 Deployment of Azure Resource Policies (JSON)\parent_PolicyTemplates\parentAzurePolicies.json' -ManagementGroupId $variables.ManagementGroupId -TemplateParameterObject $paramObject -Name $TimeNow -Verbose -ErrorAction Continue
